...
export const sendLeaveUpdateEmail = async (
  ...
  let subject = `Leave Request Update: ${status}`;
  ...
  let html = `
    <div style="font-family: Arial, sans-serif; max-w: 600px; margin: 0 auto; background: #ffffff; border-radius: 12px; overflow: hidden; border: 1px solid #e2e8f0; box-shadow: 0 10px 25px -5px rgba(0, 0, 0, 0.1);">
      <div style="background: ${color}; padding: 40px 20px; text-align: center;">
        <h1 style="color: #ffffff; margin: 0; font-size: 24px; font-weight: 800; letter-spacing: 0.5px;">Leave Request Update: ${status}</h1>
      </div>
      <div style="padding: 30px;">
        <p style="color: #334155; font-size: 16px; line-height: 1.6;">Hi ${name}, your leave request from ${startStr} to ${endStr} has been ${status.toUpperCase()} by the Admin.</p>
