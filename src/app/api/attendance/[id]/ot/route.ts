...
export async function PATCH(req: Request, { params }: { params: { id: string } }) {
  try {
    ...
    if (!record || !record.timestamp || !record.checkOut) {
    ...
    if (otStatus === 'APPROVED') {
      const totalMs = record.checkOut.getTime() - record.timestamp.getTime();
    ...
    const updated = await prisma.attendanceLog.update({
      where: { id: params.id },
      data: { otStatus, approvedOtMinutes: approvedMinutes }
    });
