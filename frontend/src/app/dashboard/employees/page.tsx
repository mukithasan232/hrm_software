  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl md:text-3xl font-bold text-slate-800 dark:text-white">Employee Directory</h1>
          <p className="text-slate-500 dark:text-gray-400 mt-1 text-sm">{employees.length} total · {employees.filter(e => e.isActive).length} active</p>
        </div>
        {canEdit && (
          <button
            onClick={openAdd}
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-xl pl-10 pr-4 py-2.5 text-slate-800 dark:text-white text-sm placeholder-slate-400 dark:placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-blue-500/50 shadow-sm"
          />
        </div>
      ...
      {loading ? (
          ...
          {[...Array(8)].map((_, i) => (
            <div key={i} className="bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-2xl p-6 animate-pulse shadow-sm">
              <div className="flex items-start justify-between">
                <div className="h-12 w-12 rounded-full bg-slate-100 dark:bg-white/10" />
                <div className="h-5 w-16 bg-slate-100 dark:bg-white/10 rounded" />
              </div>
              <div className="mt-4 space-y-2">
                <div className="h-5 w-32 bg-slate-100 dark:bg-white/10 rounded" />
                <div className="h-4 w-24 bg-slate-100 dark:bg-white/10 rounded" />
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="text-center py-20">
          ...
          <p className="text-slate-555 dark:text-gray-400">No employees found{search ? ` for "${search}"` : ''}.</p>
          {canEdit && !search && (
          ...
          {filtered.map(emp => (
            <div
              ...
              className={`bg-white dark:bg-white/5 backdrop-blur-xl border rounded-2xl p-5 transition-all group relative shadow-sm dark:shadow-md ${
                emp.isActive ? 'border-slate-200 dark:border-white/10 hover:border-blue-500/40 dark:hover:border-blue-500/40' : 'border-slate-100 dark:border-white/5 opacity-60'
              }`}
            >
              {/* Inactive badge */}
              {!emp.isActive && (
                <span className="absolute top-3 right-3 px-2 py-0.5 text-[10px] font-bold bg-red-500/20 text-red-550 dark:text-red-400 rounded-full border border-red-500/20">
                  Inactive
                </span>
              )}
              ...
              <div className="mt-3">
                <h3 className="font-bold text-slate-800 dark:text-white group-hover:text-blue-600 dark:group-hover:text-blue-300 transition-colors leading-tight">{emp.name}</h3>
                <p className="text-xs text-slate-500 dark:text-gray-400 mt-0.5">{emp.designation || 'Staff'}</p>
                <div className="flex items-center gap-1.5 mt-2">
                  <Building2 className="w-3 h-3 text-slate-400 dark:text-gray-500" />
                  <span className="text-xs text-slate-500 dark:text-slate-400">{emp.department || 'General'}</span>
                </div>
              </div>

              <div className="mt-3">
                <span className={`px-2 py-0.5 rounded-full text-[11px] font-semibold border ${ROLE_COLORS[emp.role] || ROLE_COLORS['Executive']}`}>
                  {emp.role}
                </span>
              </div>

              <div className="mt-4 pt-3 border-t border-slate-100 dark:border-white/10 flex items-center gap-1.5 text-xs text-slate-500 dark:text-gray-500">
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/15 rounded-2xl shadow-2xl w-full max-w-xl max-h-[90vh] overflow-y-auto z-10">
            {/* Modal Header */}
            <div className="sticky top-0 bg-white dark:bg-slate-900 border-b border-slate-100 dark:border-white/10 px-6 py-4 flex items-center justify-between z-10">
              <div>
                <h2 className="text-lg font-bold text-slate-800 dark:text-white">{editTarget ? 'Edit Employee' : 'Add New Employee'}</h2>
                <p className="text-xs text-slate-500 dark:text-gray-500 mt-0.5">{editTarget ? `Editing ${editTarget.name}` : 'Fill in details to create a new account'}</p>
              </div>
              <button onClick={() => setShowModal(false)} className="p-1.5 text-slate-400 hover:text-slate-800 dark:text-gray-400 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-white/10 rounded-lg transition-all">
                <X className="w-5 h-5" />
              </button>
            </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div className="space-y-1">
                  ...
                  <input
                    ...
                    disabled={!!editTarget}
                    placeholder="EMP001"
                    className="w-full bg-slate-50 dark:bg-black/30 border border-slate-200 dark:border-white/10 rounded-xl px-3 py-2.5 text-slate-800 dark:text-white text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/50 disabled:opacity-50 disabled:cursor-not-allowed font-medium"
                  />
                </div>

                {/* Role */}
                <div className="space-y-1">
                  <label className="text-xs text-slate-600 dark:text-gray-400 font-medium">Role *</label>
                  <select
                    value={form.role}
                ...
                <div className="space-y-1 sm:col-span-2">
                  <label className="text-xs text-slate-600 dark:text-gray-400 font-medium">Full Name *</label>
                  <input
                    type="text"
                    required
                    value={form.name}
                ...
                {/* Email */}
                <div className="space-y-1 sm:col-span-2">
                  <label className="text-xs text-slate-600 dark:text-gray-400 font-medium">Email Address *</label>
                  <input
                    type="email"
                    required
                    value={form.email}
                ...
                {/* Designation */}
                <div className="space-y-1">
                  <label className="text-xs text-slate-600 dark:text-gray-400 font-medium">Designation</label>
                  <input
                    type="text"
                ...
                {/* Department */}
                <div className="space-y-1">
                  <label className="text-xs text-slate-600 dark:text-gray-400 font-medium">Department</label>
                  <select
