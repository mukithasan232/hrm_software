...
const connectAndListen = async (): Promise<void> => {
  ...
  if (Date.now() < circuitBreakerUntil) {
    const waitTime = Math.ceil((circuitBreakerUntil - Date.now()) / 1000);
    console.log(`[RealtimeService] 🛑 Circuit breaker active. Waiting ${waitTime}s before retrying.`);
    if (activeReconnectTimeout) clearTimeout(activeReconnectTimeout);
  ...
  deviceMutex = deviceMutex.then(async () => {
    ...
    try {
      ...
      if (consecutiveFailures >= 3) {
        console.error(`[RealtimeService] 🛑 Circuit breaker triggered after ${consecutiveFailures} failures! Suspending realtime sync for 10 minutes.`);
      ...
      if (isListenerActive) {
        if (activeReconnectTimeout) clearTimeout(activeReconnectTimeout);
        if (consecutiveFailures >= 3) {
          activeReconnectTimeout = setTimeout(connectAndListen, 10 * 60 * 1000);
        } else {
